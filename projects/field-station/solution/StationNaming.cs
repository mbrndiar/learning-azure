...
public static partial class StationNaming
{
    ...
    public static string ArtifactName(ArtifactKey key)
    {
        ...
        //
        // Nothing time-based, nothing random. A learner who appends
        // DateTimeOffset.UtcNow here passes every single-run test and breaks
        // every duplicate test in this project, because the second delivery of
        // the same observation then writes a second blob instead of colliding
        // with the first.
        return $"{StationPrefix(key.StationId)}{key.ObservationId}.json";
    }
