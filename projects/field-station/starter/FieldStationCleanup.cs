...
public sealed class FieldStationCleanup(IArtifactStore store, IStationStatusIndex index, IWorkBacklog queue)
{
    ...
    public async Task<CleanupReport> RemoveStationAsync(string stationId, CancellationToken cancellationToken)
    {
        ...
        // the state cleanup exists to resolve. Listing by prefix is the only view
        // that includes work this process never saw.
        //
        // List with Store.ListNamesAsync over StationNaming.StationPrefix and
        // delete each name, counting what was actually removed. Then read the
        // station's status rows INTO A LIST before deleting any of them: deleting
        // from a partition while paging through the same partition is a
        // well-known way to skip rows and then report a clean teardown that is
        // not clean. Finish by reporting Queue.ApproximateDepthAsync so the
        // caller can fail on a teardown that left work behind.
        throw new NotImplementedException(
            "GAP 13: remove everything the station's run created. See "
