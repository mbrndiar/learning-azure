...
public static class ResourceNaming
{
    ...
    public static ResourceName Compose(AzureService service, string prefix, string runId)
    {
        ...
        var rule = RuleFor(service);
        var cleanPrefix = Sanitize(prefix, rule);
        var cleanRun = Sanitize(runId, rule);
        ...
        if (cleanRun.Length == 0)
        {
            return new ResourceName(
                string.Empty,
                false,
                "The run id contains no character this service allows, so two runs cannot be told apart.");
        }

        var separator = rule.AllowsHyphens ? "-" : string.Empty;
        var budget = rule.MaximumLength - cleanRun.Length - separator.Length;
        if (budget < 1)
        {
            return new ResourceName(
                cleanRun,
                ...
                FormattableString.Invariant(
                    $"The run id alone is {cleanRun.Length} characters and the limit is {rule.MaximumLength}."));
        }

        var head = cleanPrefix.Length > budget ? cleanPrefix[..budget] : cleanPrefix;
        var name = head + separator + cleanRun;

        while (name.Length < rule.MinimumLength)
        {
            name += "0";
        }

        var violation = Validate(name, rule);
        return new ResourceName(name, violation is null, violation);
    }
    ...
    public static IReadOnlyList<string> ValidateTags(ResourceTags? tags, DateOnly today)
    {
        ...
        if (tags is null)
        {
            return ["The resource carries no tags, so it cannot be attributed, budgeted, or safely deleted."];
        }

        var problems = new List<string>();
        if (string.IsNullOrWhiteSpace(tags.Owner))
        {
            problems.Add("owner is empty: nobody knows who to ask before deleting it.");
        }

        if (!string.Equals(tags.ManagedBy, ResourceTags.CourseManagedBy, StringComparison.Ordinal))
        {
            problems.Add(FormattableString.Invariant(
                $"managed-by is '{tags.ManagedBy}', not '{ResourceTags.CourseManagedBy}': teardown will refuse to touch it."));
        }

        if (string.IsNullOrWhiteSpace(tags.Purpose))
        {
            problems.Add("purpose is empty: the resource cannot be explained on a bill.");
        }

        if (tags.ExpiresOn < today)
        {
            problems.Add(FormattableString.Invariant(
                $"expires-on is {tags.ExpiresOn.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture)}, which is in the past."));
        }
