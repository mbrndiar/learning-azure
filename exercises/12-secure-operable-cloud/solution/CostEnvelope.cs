...
public static class CostEnvelope
{
        ...
        decimal budgetUsd)
    {
        ...
        var hours = (decimal)runLength.TotalHours;
        var runCost = resources.Sum(resource => resource.RatePerHourUsd * hours);

        var idlePerHour = resources
            .Where(resource => resource.Shape != BillingShape.Consumption)
            .ToList();

        var idlePerDay = idlePerHour.Sum(resource => resource.RatePerHourUsd) * HoursPerDay;

        var dominant = idlePerHour.Count == 0
            ? "nothing: every resource here is billed for what it does, not for existing"
            : idlePerHour.OrderByDescending(resource => resource.RatePerHourUsd)
                .ThenBy(resource => resource.Name, StringComparer.Ordinal)
                .First()
                .Name;

        return new CostEstimate(runCost, idlePerDay, runCost <= budgetUsd, dominant);
    }
}
