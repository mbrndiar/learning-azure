...
public sealed class ConcurrencyGuard
{
    ...
    public static bool ShouldRetry(int statusCode)
    {
        ...
        // their version — that resolves. 429 and 503 are the service asking for
        // time. 404, 409 and 400 will return exactly the same answer no matter
        // how many times they are asked, so retrying them converts a fast,
        // clear failure into a slow, identical one.
        // See lessons/11-cosmos-development/README.md#retry-is-a-budget-not-a-loop
        return statusCode is PreconditionFailed or TooManyRequests or 503 or 408;
    }
        ...
        int maximumAttempts)
    {
        ...
        for (var attempt = 1; attempt <= maximumAttempts; attempt++)
        {
            ...
            //
            // Reading once and retrying the same proposed document is the
            // defect this whole mechanism exists to prevent: every attempt
            // carries the same stale ETag, so every attempt fails with 412 and
            // the loop burns its budget achieving nothing. Worse, an
            // implementation that reacts to 412 by dropping the ETag and
            // writing unconditionally has silently reintroduced the lost update
            ...
            var proposed = change(current);

            status = store.TryReplace(proposed, current.ETag);

            if (status == Ok)
            {
                return new ConcurrencyResult(WriteOutcome.Applied, attempt, proposed.ETag, status);
            }

            if (!ShouldRetry(status))
            {
                return new ConcurrencyResult(WriteOutcome.Rejected, attempt, null, status);
            }
        }

        return new ConcurrencyResult(WriteOutcome.Exhausted, maximumAttempts, null, status);
    }
}
