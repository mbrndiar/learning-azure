...
public sealed class ArtifactWorker(IWorkBacklog queue, StationLedger ledger, int maxDeliveryCount)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // JournalCodec.DecodeWorkOrder throws FormatException or JsonException for a
        // body that will never decode; that is poison on the FIRST delivery, because
        // retrying a deterministic failure buys nothing but a queue that never
        // drains. Use QuarantineAsync for both that and a spent budget.
        //
        // Then ask the ledger what this delivery is:
        //   * AlreadyJournaled -> delete the message so it stops being delivered,
        //                         but do NOT run the handler again;
        //   * Quarantined      -> quarantine; it needs a human, not an attempt;
        //   * Claimed/Resumed  -> run the effect.
        //
        // GAP 12 — Shutdown is not a message defect.
        //
        // Swallowing OperationCanceledException here quarantines healthy work every
        // time the process stops. Let it propagate: not deleting the message IS the
