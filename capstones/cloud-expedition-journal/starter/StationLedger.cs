...
public sealed class StationLedger(IStationRegistry registry, TimeProvider clock, int maxConcurrencyRetries = 5)
{
    ...
    public Task<ClaimOutcome> TryClaimAsync(
        ObservationKey key,
        string artifactName,
        CancellationToken cancellationToken)
    {
        ...
        // Try IStationRegistry.TryInsertAsync first; a non-null ETag means this
        // caller won the race and may run the effect. On failure, read the row
        // that beat it and map its phase: Journaled -> AlreadyJournaled,
        // Quarantined -> Quarantined, Pending -> Resumed. A row that vanished
        // between the failed insert and the read is a teardown racing a worker;
        // treat it as a fresh claim rather than inventing a phase for it.
        throw new NotImplementedException(
            "GAP 6: claim one observation with a conditional insert. See "
    ...
    public Task<StationState> AdvanceWatermarkAsync(
        string stationId,
        long sequenceNumber,
        int journaledDelta,
        CancellationToken cancellationToken)
    {
        ...
        // A retry that resends a fresh ETag carrying the value computed from the
        // stale read silently reintroduces the lost update the ETag existed to
        // prevent. Both look like a working retry from the outside; only the
        // second one corrupts the count.
        //
        // Loop up to _maxConcurrencyRetries times. Each attempt re-reads the
        // watermark row, seeds it with TryInsertAsync when it does not exist yet,
        // and otherwise recomputes from the row it just read before calling
        // TryReplaceAsync with that row's ETag. Increment ReplaceAttempts on every
        // replace so the tests can see how hard the loop worked. The watermark only
        // ever moves forward: use Math.Max, because an out-of-order or replayed
        // event carries a position the row has already passed, and accepting it
        // would let a replay rewind the consumer and redeliver everything after it.
        //
        // GAP 8 — Retries are bounded.
        //
        // An unbounded loop against a hot row is an outage that presents as a
        // hang, which is the hardest kind to diagnose. Failing loudly with an
        // InvalidOperationException after the budget is spent is the behaviour an
        // operator can act on.
        throw new NotImplementedException(
            "GAP 7: advance the watermark under optimistic concurrency. See "
            + "capstones/cloud-expedition-journal/README.md#milestone-2-the-storage-workflow.");
    }
