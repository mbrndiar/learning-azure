...
public sealed class TelemetryIngress(ITelemetryFeed stream, int maxEventsPerBatch = 32)
{
    ...
    public IReadOnlyList<TelemetryBatch> Plan(IReadOnlyList<TelemetryReading> readings)
    {
        ...
        // Group by ExpeditionNaming.PartitionKey with StringComparer.Ordinal,
        // then split each group at the ceiling. A group of exactly the ceiling
        // must produce one batch, not two.
        throw new NotImplementedException(
            "GAP 5: group readings into legal batches. See "
