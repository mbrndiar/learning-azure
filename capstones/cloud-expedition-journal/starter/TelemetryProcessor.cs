    ...
    int checkpointEvery = 5)
{
    ...
    public Task<ProcessorReport> RunAsync(
        Func<StreamEvent, CancellationToken, Task> handler,
        CancellationToken cancellationToken)
    {
        // Walk _stream.GetPartitionIdsAsync and, for each partition:
        //
        // GAP 13 — Claim the partition before reading it.
        //
        // Two processors reading the same partition of the same consumer group
        // both handle every event and both checkpoint over each other. The claim
        // is a conditional write on a lease blob, so exactly one instance wins and
        // the loser finds out by losing rather than by being told. A null claim is
        // a partition this pass does not own; count it and move on.
        //
        // GAP 14 — Resume from the checkpoint, and treat what arrives twice as a
        // replay rather than as new work.
        //
        // The service resumes from a position, and positions are coarse: a
        // checkpoint at sequence 40 with a crash at 47 redelivers 41 to 47. The
        // consumer therefore needs its own watermark, in memory for this pass and
        // in the checkpoint across passes, and must compare against it BEFORE
        // handling. A partition with no checkpoint starts below every sequence
        // number the service issues.
        //
        // GAP 15 — Checkpoint AFTER the handler succeeded, never before.
        //
        // A checkpoint written before handling turns every crash into silent data
        // loss: the position says the event was handled and nothing will ever
        // deliver it again. Written after, the same crash costs a duplicate, which
        // the ledger already absorbs. Checkpoint every _checkpointEvery handled
        // events, and write a closing checkpoint for the tail the interval did not
        // cover — without it, a clean shutdown replays up to _checkpointEvery
        // events for no reason at all.
        //
        // TryWriteCheckpointAsync returning null means the lease expired and
        // someone else owns the partition now. Anything this instance keeps
        // handling is work the new owner is also doing, so stop rather than race it.
        //
        // Report every count honestly: they are the only evidence an operator has
        // that a replay was absorbed rather than reprocessed.
        throw new NotImplementedException(
            "GAP 13: own, resume, handle, and checkpoint one pass over the stream. See "
