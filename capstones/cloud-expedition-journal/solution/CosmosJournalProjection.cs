...
public static class CosmosOutcomes
{
    ...
    public static ThrottledException? AsThrottle(CosmosException error)
    {
        ...
        // RetryAfter is absent on some responses, so a floor keeps the retry loop
        // from becoming a tight spin against a service already asking for room.
        var retryAfter = error.RetryAfter is { } wait && wait > TimeSpan.Zero
            ? wait
            : TimeSpan.FromMilliseconds(50);

        return new ThrottledException(
            $"Cosmos rate limited the request (activity {error.ActivityId}).",
            retryAfter,
            error.RequestCharge);
    }
}
...
public sealed class CosmosJournalProjection(Container container) : IJournalProjection
{
    ...
    /// <inheritdoc />
    public async Task<ProjectionResult> WriteAsync(
        JournalEntry entry,
        string? ifMatch,
        CancellationToken cancellationToken)
    {
        ...
        var partitionKey = new PartitionKey(entry.StationId);
        ...
        try
        {
            if (ifMatch is null)
            {
                var created = await Container
                    .CreateItemAsync(document, partitionKey, cancellationToken: cancellationToken)
                    .ConfigureAwait(false);

                return new ProjectionResult(ProjectionOutcome.Written, created.ETag, created.RequestCharge);
            }
            ...
            return new ProjectionResult(ProjectionOutcome.Written, replaced.ETag, replaced.RequestCharge);
        }
        ...
        {
            // Somebody created the document between this caller's read and its
            // write. The caller re-reads and re-decides; it does not overwrite.
            return new ProjectionResult(ProjectionOutcome.Superseded, null, error.RequestCharge);
        }
        catch (CosmosException error) when (error.StatusCode == HttpStatusCode.PreconditionFailed)
        {
            return new ProjectionResult(ProjectionOutcome.Stale, null, error.RequestCharge);
        }
    }
