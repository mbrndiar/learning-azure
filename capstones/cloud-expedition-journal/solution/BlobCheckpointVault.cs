    ...
    TimeSpan leaseDuration) : ICheckpointStore
{
    ...
    /// <inheritdoc />
    public async Task<PartitionOwnership?> TryClaimAsync(
        string partitionId,
        string ownerId,
        CancellationToken cancellationToken)
    {
        ...
        catch (RequestFailedException error) when (error.Status == 404)
        {
            ...
            //
            // Two processors starting together both find no blob. Only a
            // conditional create lets the service decide which of them owns the
            // partition; a plain upload lets both believe they do.
            try
            {
                var created = await blob.UploadAsync(
                    new BinaryData("{}").ToStream(),
                    new BlobUploadOptions
                    {
                        Conditions = new BlobRequestConditions { IfNoneMatch = ETag.All },
                        Metadata = Claim(ownerId, now),
                        HttpHeaders = new BlobHttpHeaders { ContentType = "application/json" },
                    },
                    cancellationToken).ConfigureAwait(false);

                return new PartitionOwnership(partitionId, ownerId, created.Value.ETag.ToString("H"));
            }
            catch (RequestFailedException conflict) when (conflict.Status is 409 or 412)
            {
                return null;
            }
        }

        var holder = properties.Metadata.TryGetValue(OwnerMetadataKey, out var owner) ? owner : string.Empty;
        var claimedAt = ParseInstant(
            properties.Metadata.TryGetValue(ClaimedAtMetadataKey, out var claimed) ? claimed : null);
        var expired = claimedAt is null || now - claimedAt.Value >= LeaseDuration;

        if (!expired && !string.Equals(holder, ownerId, StringComparison.Ordinal))
        {
            // Somebody else holds a live lease. Taking it anyway is how two
            // processors end up handling the same partition and checkpointing
            // over each other.
            return null;
        }
        ...
        try
        {
            ...
            // expired lease both take it, which is precisely the race the lease
            // was supposed to settle.
            var renewed = await blob.SetMetadataAsync(
                Claim(ownerId, now),
                new BlobRequestConditions { IfMatch = properties.ETag },
                cancellationToken).ConfigureAwait(false);

            return new PartitionOwnership(partitionId, ownerId, renewed.Value.ETag.ToString("H"));
        }
