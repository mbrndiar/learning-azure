...
public static class TelemetryEventMapper
{
    ...
    public static StreamEvent? ToStreamEvent(PartitionEvent partitionEvent)
    {
        ...
        var reading = JournalCodec.DecodeReading(Encoding.UTF8.GetString(data.EventBody.ToArray()));

        return new StreamEvent(
            partitionEvent.Partition.PartitionId,
            data.SequenceNumber,
            data.OffsetString ?? data.SequenceNumber.ToString(System.Globalization.CultureInfo.InvariantCulture),
            data.PartitionKey ?? ExpeditionNaming.PartitionKey(reading.Key),
            reading);
    }
}
