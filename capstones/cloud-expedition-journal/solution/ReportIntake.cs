...
public sealed class ReportIntake(IArtifactVault vault, TimeProvider clock)
{
    ...
    public async Task<IntakeResult> PreserveAsync(TelemetryReading reading, CancellationToken cancellationToken)
    {
        ...
        var result = await _vault
            .CreateIfAbsentAsync(name, content, ReportContentType, cancellationToken)
            .ConfigureAwait(false);

        return result.Outcome switch
        {
            WriteOutcome.Written => new IntakeResult(IntakeOutcome.Stored, key, name, result.ETag),
            WriteOutcome.AlreadyExists => new IntakeResult(IntakeOutcome.Duplicate, key, name, null),
...
public sealed class WorkDispatcher(IWorkBacklog queue)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (intake.Outcome != IntakeOutcome.Stored)
        {
            return false;
        }

        var order = new ArtifactWorkOrder(
            ExpeditionNaming.WorkOrderId(intake.Key, operation),
            intake.Key.StationId,
            intake.Key.ObservationId,
            intake.ArtifactName,
            operation);

        await _queue.SendAsync(order, cancellationToken).ConfigureAwait(false);
        return true;
    }
}
